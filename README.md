...
> 46번의 실험, 27일간의 기록.
> **Private 1위 (BA 0.94821) · Public BA 0.9995**
...
입력: 8,500건 (이미지 + 텍스트 맥락 + 질문 + 3지선다)
  ...
  │
  └─ 출력: submission.csv (~36분, 70분 제한의 절반)
```
...
![B패밀리 카테고리별 Commit Rate](docs/charts_v42/3_b_category.png)

v42의 시각편향 경고가 emotional, authority 카테고리에서 commit rate를 효과적으로 낮춘 것을 확인할 수 있습니다.

**실제 테스트 샘플 — 스톡사진 편향 사례:**

![v42 샘플 이미지](docs/charts_v42/4_sample_images.png)
...
![v39 Pipeline Waterfall](docs/charts_v39/1_waterfall.png)
...
![v44 Analysis](docs/v44_analysis.png)

### 4.7 [v46] permSC 합의 검증 — 실패 사례

v46에서는 모든 변경을 **permSC(3-순열 + 중재)로 독립 교차검증**하여, 양쪽 동의하는 변경만 유지하는 전략을 시도했습니다.

**결과: Public BA 0.9993 (v44 대비 -0.0002) → 폐기.**

실패 원인: permSC가 base SYSTEM_PROMPT로 검증하기 때문에, Recovery 프롬프트가 제공하는 "Key sentence: ..." 힌트 없이는 ATTR 패턴을 인식하지 못합니다. A패밀리 Recovery의 81%가 원복되어 오히려 성능이 하락했습니다.
...
| **v44** | **0.9995** | + ATTR fix + B-Recovery + Over-commit 억제 **(Private 1위, BA 0.94821)** |
| v46 | 0.9993 | + permSC 합의 검증 (과도한 검증으로 A패밀리 퇴화 → 폐기) |

**v42 Pipeline Waterfall — 각 레버의 기여:**

![v42 Pipeline Waterfall](docs/charts_v42/1_waterfall.png)
...
![Face Crop Effect](docs/charts_v43/4_face_crop_effect.png)

**v43 Pipeline Waterfall — 논문 기법 적용 결과:**

![v43 Pipeline Waterfall](docs/charts_v43/1_waterfall.png)
...
├── colab_v44_submission.ipynb      # 최종 제출 노트북 (추론 전용, 10셀)
├── colab_v44_final.ipynb          # v44 개발 노트북 (분석/벤치마크 포함)
├── scripts/
│   ├── make_v44_submission.py     # 제출 노트북 생성 스크립트
│   ├── make_v44_final.py
...
- 최종 제출: v44 (`colab_v44_submission.ipynb`)
- 모델: Qwen/Qwen3.5-9B (학습 없음, 프롬프트 엔지니어링만)
- v46(permSC)은 과검증으로 폐기, v44가 Private에서도 최고 성능
