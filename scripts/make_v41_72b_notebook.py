# v41-72B = v41 파이프라인 + Qwen3-VL-72B-Instruct FP8 (G4 100GB GPU용)
import json

SRC = r"C:\Users\pak10\Downloads\colab_v31_grounding_off.ipynb"
OUT = r"c:\Pyg\Projects\dacon\SKKU-Multimodal-Challenge-2026\colab_v41_72b_fp8.ipynb"
...
body1 = body1.replace(
    ...
    '_kw = dict(model=MODEL, dtype="auto", max_model_len=4096,\n           quantization="fp8", gpu_memory_utilization=0.95,'
)
...
# ────────────────────────────────────────────
INTRO = """# v41-72B — Qwen3-VL-72B-Instruct FP8 + v41 파이프라인

**모델:** Qwen3-VL-72B-Instruct (FP8 양자화, ~75GB VRAM)
**GPU:** G4 100GB 이상 필요. A6000 48GB에서는 실행 불가.

**파이프라인 (v41과 동일):**
1. **base 1패스** 8500건 (768px)
2. **반사실 1패스** A집단치환 + B성별치환 (~3500)
3. **LLM 중재 디바이어싱** 충돌 시 LLM 재판단
4. **A-family LLM recovery** 잔여 unknown에 강화 프롬프트 재질문
5. **분석 시각화** (차트 4장)

**주의:** 72B 모델이라 9B 대비 추론 느림. 전체 ~2시간+ 예상.
**산출:** `outputs/submission_v41_72b_fp8.csv` + 분석 차트.
**실행:** 셀0 설치→재시작→순서대로.
"""
...
CHART_DIR=f'{PROJECT}/outputs/charts_v41_72b'
...
fig.suptitle('v41-72B Pipeline Waterfall (Qwen3-VL-72B FP8)',fontsize=14,fontweight='bold')
...
checks = [
    ...
    ("VER=v41_72b", "v41_72b_fp8" in FINAL_CMP),
    ("v40 비교 있음", "v40" in FINAL_CMP),
    ("차트 v41_72b", "charts_v41_72b" in ANALYSIS),
