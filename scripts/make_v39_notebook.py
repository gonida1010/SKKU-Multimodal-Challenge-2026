# v39 = v38 구조 + Recovery 삭제 → ATTR disambig override.
# API 분석(GPT+Gemini) 결과: A패밀리 disambig에서 행동 주체(tgt)가 99.7% 정답.
# recovery 48분 → 0초, +132건 순 개선 (API ground truth 대비).
import json

SRC = r"C:\Users\pak10\Downloads\colab_v31_grounding_off.ipynb"
OUT = r"c:\Pyg\Projects\dacon\SKKU-Multimodal-Challenge-2026\colab_v39_attr_override.ipynb"
...
# ────────────────────────────────────────────
INTRO = """# v39 — 반사실 디바이어싱 + ATTR Disambig Override
...
# ────────────────────────────────────────────
ATTR_OVERRIDE = r"""# ===== E: ATTR Disambig Override (recovery 대체, 0초) =====
...
# ────────────────────────────────────────────
ANALYSIS = r"""# ===== G: 분석 시각화 (차트 4장 자동 생성) =====
...
cells = [
    ...
    code(body(3)),           # 셀3: drive mount (v31 body(3))
    code(DATA),              # 셀A: 데이터 + 768 이미지
    code(CFBUILD),           # 셀B: 반사실 생성
    code(RUN),               # 셀C: 1패스 base + CF
    code(DEBIAS),            # 셀D: 디바이어싱 규칙 → unk_mask
    code(ATTR_OVERRIDE),     # 셀E: ATTR disambig override (recovery 대체)
    code(FINAL_CMP),         # 셀F: 저장 + synth_gold 비교
    code(ANALYSIS),          # 셀G: 분석 시각화 4장
]
...
# 검증
checks = [
    ("ATTR_RE", "ATTR_RE=re.compile" in ATTR_OVERRIDE),
    ("override_count", "override_count" in ATTR_OVERRIDE),
    ("final=list(debiased)", "final=list(debiased)" in ATTR_OVERRIDE),
    ("fam() 사용", "fam(rows[k]" in ATTR_OVERRIDE),
    ("tgt 할당", "final[k]=tgt" in ATTR_OVERRIDE),
    ("recovery 없음", "recovery" not in "".join(c.get("source","") if isinstance(c.get("source"),str) else "".join(c.get("source",[])) for c in cells)),
    ("VER=v39", "v39_attr_override" in FINAL_CMP),
    ("INTRO 있음", "v39" in INTRO),
    ("T_START 있음", "T_START" in RUN),
    ("synth_gold 있음", "synth_gold" in FINAL_CMP),
    ("분석 차트 4장", "Chart 4" in ANALYSIS),
    ("총 셀 수", len(cells) == 12),
]
