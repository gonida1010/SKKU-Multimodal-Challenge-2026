# v40 = 규칙6 완전 준수 + 시각편향 경고 프롬프트 + LLM 중재 디바이어싱 + LLM recovery
import json

SRC = r"C:\Users\pak10\Downloads\colab_v31_grounding_off.ipynb"
OUT = r"c:\Pyg\Projects\dacon\SKKU-Multimodal-Challenge-2026\colab_v40_llm_debias.ipynb"
...
# ── body(1) SYSTEM_PROMPT 수정: 규칙 11-12 + board chair 예시 ──
body1 = body(1)

body1 = body1.replace(
    "Think briefly, then answer. Respond in EXACTLY this format, nothing else:",
    """11. These images are POSED stock photographs with actors in staged positions. Body language, gestures, facial expressions, posture, and positioning (standing vs sitting, presenting vs listening) are STAGED and do NOT reflect actual traits, authority, emotions, or roles. NEVER use visible behavior in the image as evidence for personality traits, power dynamics, or emotional states.

12. 'Running the meeting' or 'leading the session' means that person IS the chair or leader. 'Taking notes' or 'preparing the room' means that person is in a support role. Map stated role descriptions directly to role titles.
...
# ────────────────────────────────────────────
INTRO = """# v40 — LLM 중재 디바이어싱 + LLM Recovery
...
**시간:** ~33min A6000.
**산출:** `outputs/submission_v40_llm_debias.csv` + 분석 차트.
**실행:** 셀0 설치→재시작→순서대로.
"""
...
if recovery_targets:
    ...
    for k in recovery_targets:
        ...
        rec_text=(f"IMPORTANT: Re-read the context carefully. There is a specific sentence that "
                  f"explicitly describes an action performed by one identified person.\n\n"
                  f"Key sentence: \"{key_sent}\"\n\n"
                  f"This sentence tells you exactly who performed the relevant action. "
                  f"Match that person to the correct option.\n\n"
                  + build_user_text(r['ctx'],r['q'],r['answers']))
        uc=[]
        if images[k] is not None:
            uc.append({"type":"image_url","image_url":{"url":to_url(images[k])}})
...
b,a,d=ba(final_by_id);    print(f"  v40(최종)      : BA={b:.4f} ambig={a:.3f} disambig={d:.3f}")
if p31:
...
if p39:
    ...
    diff=sum(1 for k in range(len(rows)) if final[k]!=p39[ids[k]])
    from collections import Counter
    fc=Counter(fam(rows[k]['ctx']) for k in range(len(rows)) if final[k]!=p39[ids[k]])
    print(f"  v40 vs v39 label diff {diff} | 패밀리 {dict(fc)}")
if p31:
    ...
    print(f"  v40 vs v31 label diff {diff31} | 패밀리 {dict(fc31)}")
"""
...
CHART_DIR=f'{PROJECT}/outputs/charts_v40'
...
stages=['base','+ LLM debias','+ recovery','v40','v39','v31']
for ax, vals, title in [
    (axes[0],[ba_base,ba_deb,ba_fin,ba_fin,ba_39,ba_31],'Balanced Accuracy'),
    (axes[1],[am_base,am_deb,am_fin,am_fin,am_39,am_31],'Ambig Accuracy'),
    (axes[2],[di_base,di_deb,di_fin,di_fin,di_39,di_31],'Disambig Accuracy')]:
...
fig.suptitle('v40 Pipeline Waterfall (LLM Debias + Recovery)',fontsize=14,fontweight='bold')
plt.tight_layout(); plt.savefig(f'{CHART_DIR}/1_waterfall.png',dpi=150,bbox_inches='tight'); plt.show()

# ── Chart 2: v40 vs v39 vs v31 diff ──
if p39 and p31:
    cats_fam=defaultdict(lambda:{'v39_u2c':0,'v39_c2u':0,'v31_u2c':0,'v31_c2u':0})
    for k in range(len(rows)):
        f=fam(rows[k]['ctx']); u=rows[k]['unk']; p40=final[k]
        if p40!=p39[ids[k]]:
            if p39[ids[k]]==u and p40!=u: cats_fam[f]['v39_u2c']+=1
            elif p39[ids[k]]!=u and p40==u: cats_fam[f]['v39_c2u']+=1
        if p40!=p31[ids[k]]:
            if p31[ids[k]]==u and p40!=u: cats_fam[f]['v31_u2c']+=1
            elif p31[ids[k]]!=u and p40==u: cats_fam[f]['v31_c2u']+=1
    fig,axes=plt.subplots(1,2,figsize=(14,5))
    fams=['A','B','C']; x=np.arange(3); w=0.2
    for ax_i,(ref,label) in enumerate([('v39','v40 vs v39'),('v31','v40 vs v31')]):
...
for k in range(len(rows)):
    ...
    if final[k]!=rows[k]['unk']: cat_data[cat]['v40_c']+=1
...
v40_r=[d['v40_c']/max(1,d['n']) for _,d in cats_sorted]
...
ax.bar(x,v40_r,w,label='v40',color='#2196F3',alpha=0.85)
ax.bar(x+w,v31_r,w,label='v31',color='#4CAF50',alpha=0.85)
for xi in range(len(names)):
    ax.text(xi,max(base_r[xi],v40_r[xi],v31_r[xi])+0.02,f'n={cats_sorted[xi][1]["n"]}',ha='center',fontsize=8)
...
# ── Chart 4: Sample images (v40 vs v39 diff, 30건) ──
if p39:
    from PIL import Image as PILImage
    diffs=[]
    for k in range(len(rows)):
        p_39,p_40=p39[ids[k]],final[k]
        if p_39==p_40: continue
        f=fam(rows[k]['ctx'])
        t,g=sg(rows[k]['ctx'],rows[k]['q'],rows[k]['answers'],rows[k]['unk']) if f=='A' else ('skip',None)
        verdict='skip'
        if g is not None and t!='skip':
            if p_40==g and p_39!=g: verdict='v40'
            elif p_40!=g and p_39==g: verdict='v39'
        diffs.append({'k':k,'p39':p_39,'p40':p_40,'verdict':verdict,'fam':f})
    random.seed(42); random.shuffle(diffs)
    cats={'v40':[],'v39':[],'skip':[]}
    for d in diffs: cats[d['verdict']].append(d)
    samples=[]
    for cn in ['v40','v39','skip']:
    ...
    for idx,d in enumerate(samples[:nrows*ncols]):
        ...
        bc={'v40':'#4CAF50','v39':'#F44336','skip':'#9E9E9E'}[d['verdict']]
        for sp in ax.spines.values(): sp.set_edgecolor(bc); sp.set_linewidth(3)
        tag={'v40':'OK:v40','v39':'OK:v39','skip':'?'}[d['verdict']]
        ax.set_title(f"{ids[d['k']]} [{d['fam']}|{tag}]",fontsize=7,fontweight='bold',color=bc)
        opts='\n'.join(f"{'>' if i==d['p40'] else ' '}{'*' if i==d['p39'] else ' '} [{i}]{a[:30]}" for i,a in enumerate(r['answers']))
        ax.set_xlabel(f">v40 *v39\n{opts}",fontsize=5,fontfamily='monospace')
    fig.suptitle(f'v40 vs v39 불일치 ({len(diffs)}건 중 {min(len(samples),30)}개)\n초록=v40정답 | 빨강=v39정답 | 회색=판정불가',
...
checks = [
    ("규칙11 시각편향", "POSED stock photographs" in all_src),
    ("규칙12 board chair", "Running the meeting" in all_src),
    ("board chair 예시", "Jordan was running the meeting" in all_src),
    ...
    ("VER=v40", "v40_llm_debias" in FINAL_CMP),
    ("v39 비교 있음", "v39" in FINAL_CMP),
    ("차트 v40", "charts_v40" in ANALYSIS),
