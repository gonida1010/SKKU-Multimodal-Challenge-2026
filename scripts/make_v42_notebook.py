...
# API 예측 로드
gem=gpt=None
for d in [f'{PROJECT}/핸드오프', f'{PROJECT}/outputs', PROJECT]:
    gp=f'{d}/api_pred_gemini.csv'
    if os.path.exists(gp) and gem is None:
        gem={r['sample_id']:int(r['label']) for r in csv.DictReader(open(gp,encoding='utf-8'))}
        print(f"Gemini 로드: {gp} ({len(gem)}건)")
    gp2=f'{d}/api_pred_gpt4o.csv'
    if os.path.exists(gp2) and gpt is None:
        gpt={r['sample_id']:int(r['label']) for r in csv.DictReader(open(gp2,encoding='utf-8'))}
        print(f"GPT-4o 로드: {gp2} ({len(gpt)}건)")
if gem is None: print("Gemini CSV 없음 - 핸드오프/ 또는 outputs/에 api_pred_gemini.csv 배치 필요")
if gpt is None: print("GPT-4o CSV 없음 - 핸드오프/ 또는 outputs/에 api_pred_gpt4o.csv 배치 필요")
...
if gem:
    agree_gem=sum(1 for k in range(len(rows)) if final[k]==gem.get(ids[k]))
    print(f"  v42 vs Gemini: 일치 {agree_gem}/8500 ({agree_gem/85:.1f}%) diff {8500-agree_gem}")
if gpt:
    agree_gpt=sum(1 for k in range(len(rows)) if final[k]==gpt.get(ids[k]))
    print(f"  v42 vs GPT-4o: 일치 {agree_gpt}/8500 ({agree_gpt/85:.1f}%) diff {8500-agree_gpt}")
if gem and gpt:
    api_agree_not_v42=sum(1 for k in range(len(rows)) if gem.get(ids[k])==gpt.get(ids[k]) and final[k]!=gem.get(ids[k]))
    print(f"  API합의(Gem=GPT) but v42 다름: {api_agree_not_v42}건")
    all3=sum(1 for k in range(len(rows)) if final[k]==gem.get(ids[k])==gpt.get(ids[k]))
    print(f"  v42=Gemini=GPT-4o 3자일치: {all3}/8500 ({all3/85:.1f}%)")
"""
...
# ── Chart 4: Multi-model consensus (API 비교) ──
if gem or gpt:
    fig,axes=plt.subplots(1,2,figsize=(16,5))
    versions_dict={'v42':final_by_id}
    if p40: versions_dict['v40']=p40
    if p41_csv: versions_dict['v41']=p41_csv
    if p31: versions_dict['v31']=p31
    labels_v=list(versions_dict.keys())
    agree_gem_list=[]; agree_gpt_list=[]; agree_both_list=[]
    for vn in labels_v:
        vp=versions_dict[vn]
        ag=sum(1 for k in range(len(rows)) if vp.get(ids[k])==gem.get(ids[k])) if gem else 0
        agp=sum(1 for k in range(len(rows)) if vp.get(ids[k])==gpt.get(ids[k])) if gpt else 0
        ab=sum(1 for k in range(len(rows)) if gem and gpt and vp.get(ids[k])==gem.get(ids[k])==gpt.get(ids[k])) if (gem and gpt) else 0
        agree_gem_list.append(ag); agree_gpt_list.append(agp); agree_both_list.append(ab)
    x=np.arange(len(labels_v)); w=0.25
    if gem: axes[0].bar(x-w,agree_gem_list,w,label='=Gemini',color='#4285F4')
    if gpt: axes[0].bar(x,agree_gpt_list,w,label='=GPT-4o',color='#10A37F')
    if gem and gpt: axes[0].bar(x+w,agree_both_list,w,label='=Both API',color='#FF6D00')
    for xi in range(len(labels_v)):
        if gem and gpt: axes[0].text(xi+w,agree_both_list[xi]+20,str(agree_both_list[xi]),ha='center',fontsize=8)
    axes[0].set_xticks(x); axes[0].set_xticklabels(labels_v,fontsize=10)
    axes[0].set_ylabel('Agreement (of 8500)'); axes[0].set_title('각 버전의 API 모델 일치율',fontsize=12,fontweight='bold')
    axes[0].legend(fontsize=9)

    if gem and gpt:
        fam_data_api={'A':{'agree':0,'api_only':0,'v42_only':0,'split':0,'total':0},
                      'B':{'agree':0,'api_only':0,'v42_only':0,'split':0,'total':0},
                      'C':{'agree':0,'api_only':0,'v42_only':0,'split':0,'total':0}}
        for k in range(len(rows)):
            f=fam(rows[k]['ctx']); sid=ids[k]
            fam_data_api[f]['total']+=1
            gv,gpv=gem.get(sid),gpt.get(sid)
            v42v=final[k]
            if gv==gpv:
                if v42v==gv: fam_data_api[f]['agree']+=1
                else: fam_data_api[f]['api_only']+=1
            else:
                if v42v==gv or v42v==gpv: fam_data_api[f]['v42_only']+=1
                else: fam_data_api[f]['split']+=1
        fams_list=['A','B','C']
        x2=np.arange(3); w2=0.18
        axes[1].bar(x2-1.5*w2,[fam_data_api[f]['agree']/fam_data_api[f]['total'] for f in fams_list],w2,label='v42=API합의',color='#4CAF50')
        axes[1].bar(x2-0.5*w2,[fam_data_api[f]['api_only']/fam_data_api[f]['total'] for f in fams_list],w2,label='API합의 but v42 다름',color='#F44336')
        axes[1].bar(x2+0.5*w2,[fam_data_api[f]['v42_only']/fam_data_api[f]['total'] for f in fams_list],w2,label='API분열(v42=한쪽)',color='#FF9800')
        axes[1].bar(x2+1.5*w2,[fam_data_api[f]['split']/fam_data_api[f]['total'] for f in fams_list],w2,label='3자분열',color='#9E9E9E')
        for xi,f in enumerate(fams_list):
            n=fam_data_api[f]['api_only']
            if n>0: axes[1].text(xi-0.5*w2,fam_data_api[f]['api_only']/fam_data_api[f]['total']+0.005,str(n),ha='center',fontsize=8,color='#F44336')
        axes[1].set_xticks(x2); axes[1].set_xticklabels(fams_list,fontsize=11)
        axes[1].set_ylabel('Ratio'); axes[1].set_title('패밀리별 v42 vs API 합의',fontsize=12,fontweight='bold')
        axes[1].legend(fontsize=7,loc='upper right')
    plt.tight_layout(); plt.savefig(f'{CHART_DIR}/4_api_consensus.png',dpi=150,bbox_inches='tight'); plt.show()
else:
    print("API CSV 없음 - Chart 4 스킵")

# ── Chart 5: Sample images (v42 vs v40 diff, 30건) ──
if p40:
