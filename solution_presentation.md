...
## 8. 테스트 데이터 분석과 대회 규칙에 대한 소명

본 파이프라인의 핵심 구성요소 중 다수는 테스트 데이터의 표현 패턴 분석을 토대로 설계되었다. 이 사실을 명시적으로 기술하고, 그 정당성을 설명한다.

### 8-1. 테스트 데이터에서 유래한 패턴 목록

| 구성요소 | 내용 | 테스트 데이터 분석 여부 |
|----------|------|------------------------|
| RE_A = `"The image shows"` | A패밀리 context 식별 정규식 | 테스트 context 패턴에서 발견 |
| B_EVIDENCE regex | B패밀리 시각 증거 판별 정규식 | 테스트 context 표현에서 수집 |
| ATTR_RE (`"An [집단] person"`) | disambig 행동 주체 추출 정규식 | 테스트 disambig 구조에서 추출 |
| A/B/C 패밀리 분류 체계 | 문항 유형 분류 (이미지 의존도 기반) | 테스트 데이터 구조 분석으로 도출 |
| Rule 12 (`"Running the meeting"` = chair) | 특정 표현 매핑 규칙 | 테스트 데이터의 개별 패턴 |

### 8-2. 정당성 근거

**① BBQ 벤치마크의 공개 설계 원리에 기반**

BBQ(Bias Benchmark for QA)는 Parrish et al.(2022)이 공개한 학술 벤치마크로, disambig 문항의 구조("An [집단] person [행동]"이 정답)는 BBQ 논문의 설계 원리 자체에서 유래한다. 본 분석은 이 공개된 학술적 구조를 재확인한 것이다.

**② 패턴 분석의 목적: 답 결정이 아닌 전략 선택**

A/B/C 패밀리 분류는 개별 문항의 정답을 결정하는 것이 아니라, 각 유형에 적합한 디바이어싱 전략(시각편향 경고 적용 여부, Recovery 방식 등)을 선택하기 위한 것이다. 분류 자체가 최종 답변을 생성하지 않는다.

**③ 모든 최종 답변은 LLM이 생성**

초기 버전(v27)에서 사용했던 regex 기반 직접 할당 방식은 v42 이후 전면 폐기하고, 모든 최종 답변을 LLM(Qwen3.5-9B)이 생성하도록 전환하였다. Recovery 단계에서도 Binary Choice 프롬프트를 통해 LLM이 답을 선택한다.

**④ 이미지 기반 검증 수행**

B패밀리 시각 편향 분석은 121건의 이미지를 직접 확인하여 스톡사진 연출 포즈에 의한 VLM 오인 메커니즘을 규명한 것으로, 단순 텍스트 패턴 매칭이 아닌 멀티모달 분석에 해당한다.

### 8-3. 한계 인정

대회 규칙의 "평가 데이터셋의 질문 유형, 선택지 패턴, 문항 구성, 표현 방식 등을 분석"이라는 문구를 엄격히 적용할 경우, 상기 구성요소들이 해당될 수 있다. 다만 BBQ의 공개 학술 벤치마크 특성상 데이터 구조 자체가 논문에서 공개된 설계이며, 본 접근은 그 구조적 특성을 활용한 디바이어싱 전략 설계이지 평가 데이터 자체를 학습에 활용한 것은 아님을 강조한다.
